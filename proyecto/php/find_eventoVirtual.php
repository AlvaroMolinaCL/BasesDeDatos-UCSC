...
if (isset($_REQUEST['idEventoVirtual']) && $_REQUEST['idEventoVirtual'] !== '') {
    ...
    categoria, restriccion, plataforma
    FROM evento
    INNER JOIN evento_virtual
    ON evento.id=evento_virtual.fk_id WHERE id=$idEventoVirtual" ;
    $consulta = pg_query($conexion, $query);

    if (pg_num_rows($consulta) > 0) {
        if ($obj = pg_fetch_object($consulta)) {
            echo 'Datos del evento virtual con id: ' . $obj->id . '<br><br>';
            echo '<table border="1">' .
